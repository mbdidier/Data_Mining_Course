...
## Fitted Values as Linear Combinations of the Response
...
This result shows that each fitted value \(\hat{y}_i\) is a **linear combination of the observed response values** \(y_1, \dots, y_n\).  
The weights depend only on the predictor values \(x_i\), which reflects the linear nature of least squares regression.

In matrix terms, this corresponds to the hat matrix for regression through the origin.
